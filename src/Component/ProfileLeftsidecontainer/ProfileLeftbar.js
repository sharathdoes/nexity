...
export default function ProfileLeftbar() {
  ...
  const handleFollow = async()=>{
    if(Follow === "Follow"){
      await fetch(`http://localhost:3000/api/user/following/${id}` , {method:'PUT', headers:{'Content-Type':"application/JSON" , token:accessToken} , body:JSON.stringify({user:`${user.other._id}`})})
