...
export default function Post({post}) {
  ...
  useEffect(() => {
    const getuser = async()=>{
      try {
        const res  = await axios.get(`http://localhost:3000/api/user/post/user/details/${post.user}`)
        setuser(res.data);
      } catch (error) {
  ...
  const handleLike = async() => {
    if (Like == LikeIcon) {
      await fetch(`http://localhost:3000/api/post/${post._id}/like` , {method:"PUT" , headers:{'Content-Type':"application/Json" , token:accessToken}})
  ...
  const addComment = async() => {
    ...
    await fetch(`http://localhost:3000/api/post/comment/post` , {method:"PUT" , headers:{'Content-Type':"application/Json" , token:accessToken} , body:JSON.stringify(comment)})
